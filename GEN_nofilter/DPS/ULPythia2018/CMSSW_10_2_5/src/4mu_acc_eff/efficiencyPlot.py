...
# ptBin = array('d', [10, 20, 30, 40])# dummy
# yBin = array('d', [-2, -1, 0, 1, 2])# dummy
...
inFileDir = "./"
# inFileList = ["1/DPS_2018_Psi2SJ_nofilter_{}.root".format(i) for i in range(1, 11)] \
# + ["2/DPS_2018_Psi2SJ_nofilter_{}.root".format(i) for i in range(11, 21)] \
# + ["3/DPS_2018_Psi2SJ_nofilter_{}.root".format(i) for i in range(21, 31)] \
# + ["4/DPS_2018_Psi2SJ_nofilter_{}.root".format(i) for i in range(31, 46)] \
# + ["5/DPS_2018_Psi2SJ_nofilter_{}.root".format(i) for i in range(46, 61)]
# inFileList = ["8/DPS_2018_Psi2SJ_{}.root".format(i) for i in range(1, 10)]
inFileList = ["MixedDPS.root"]# Event mixing sample
inFiles = {}
inTrees = []
for inFileName in inFileList:
    inFiles[inFileName] = TFile(inFileDir + inFileName, "READ")
    # inTrees.append(inFiles[inFileName].Get("GenAnalyzer/gen_tree"))
    inTrees.append(inFiles[inFileName].Get("data"))# For event mixing sample only
...
rJpsi_pt, rJpsi_y = "GENjpsi_pt[0]", "GENjpsi_y[0]"
rJpsi_mupt, rJpsi_mueta = "GENjpsi_mu_pt[0]", "GENjpsi_mu_eta[0]"
rpsi2S_pt, rpsi2S_y = "GENpsi2s_pt[0]", "GENpsi2s_y[0]"
rpsi2S_mupt, rpsi2S_mueta = "GENpsi2s_mu_pt[0]", "GENpsi2s_mu_eta[0]"
...
for ptIdx in range(len(ptBin) - 1):
    for yIdy in range(len(yBin) - 1):
        ...
        for inTree in inTrees:
            nBin_Jpsi += inTree.GetEntries(binCut_Jpsi)# + fidCut_psi2S
            nEta_Jpsi += inTree.GetEntries(binCut_Jpsi + etaCut_Jpsi)
            nPt_Jpsi += inTree.GetEntries(binCut_Jpsi + etaCut_Jpsi + " && {}[0] > 3.5 && {}[1] > 3.5".format(rJpsi_mupt, rJpsi_mupt))
            nBin_psi2S += inTree.GetEntries(binCut_psi2S)
            nEta_psi2S += inTree.GetEntries(binCut_psi2S + etaCut_psi2S)
            nPt_psi2S += inTree.GetEntries(binCut_psi2S + etaCut_psi2S + " && {}[0] > 3.5 && {}[1] > 3.5".format(rpsi2S_mupt, rpsi2S_mupt))
