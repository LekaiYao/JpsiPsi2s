...
for ptIdx in range(len(ptBin) - 1):
    for yIdy in range(len(yBin) - 1):
        ...
        for inTree in inTrees:
            ...
            nPt_Jpsi += inTree.GetEntries(binCut_Jpsi + etaCut_Jpsi + " && {}[0] > 3.6 && {}[1] > 3.6".format(rJpsi_mupt, rJpsi_mupt))
            nBin_psi2S += inTree.GetEntries(binCut_psi2S)
            nEta_psi2S += inTree.GetEntries(binCut_psi2S + etaCut_psi2S)
            nPt_psi2S += inTree.GetEntries(binCut_psi2S + etaCut_psi2S + " && {}[0] > 3.6 && {}[1] > 3.6".format(rpsi2S_mupt, rpsi2S_mupt))
