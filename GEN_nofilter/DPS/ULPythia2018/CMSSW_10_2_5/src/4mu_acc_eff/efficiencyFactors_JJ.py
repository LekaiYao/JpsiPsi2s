...
def geteffs(channel, List, obs_bins_pt, obs_bins_y, obs_gen_pT, obs_gen_y, genbin_pt, genbin_y):
    ...
    for Sample in List:
        ...
        cut_charm_gen_pta = "(GENjpsi_pt[0] > "+str(obs_gen_low_pt)+" && GENjpsi_pt[0] < "+str(obs_gen_high_pt) + ")"# a corresponds to J/psi
    	cut_charm_gen_yb = "(GENpsi2s_eta[0] > "+str(obs_gen_low_y)+" && GENpsi2s_eta[0] < "+str(obs_gen_high_y) + ")"
    	cut_charm_gen_ptb = "(GENpsi2s_pt[0] > "+str(obs_gen_low_pt)+" && GENpsi2s_pt[0] < "+str(obs_gen_high_pt) + ")"# b corresponds to psi(2S)
        
    	cut_mu_gen_eta_a = "(abs(GENjpsi_mu_eta[0][0]) < "+str(gen_mu_eta_cut)+" && abs(GENjpsi_mu_eta[0][1]) < "+str(gen_mu_eta_cut)+")"
        cut_mu_gen_pt_a = "(GENjpsi_mu_pt[0][0] > "+str(gen_mu_pt_cut)+" && GENjpsi_mu_pt[0][1] > "+str(gen_mu_pt_cut)+" && GENjpsi_mu_pt[0] > "+str(gen_mu_pt_cut_tight)+")"
    	cut_mu_gen_eta_b = "(abs(GENpsi2s_mu_eta[0][0]) < "+str(gen_mu_eta_cut)+" && abs(GENpsi2s_mu_eta[0][1]) < "+str(gen_mu_eta_cut)+")"
        cut_mu_gen_pt_b = "(GENpsi2s_mu_pt[0][0] > "+str(gen_mu_pt_cut)+" && GENpsi2s_mu_pt[0][1] > "+str(gen_mu_pt_cut)+" && GENpsi2s_mu_pt[0] > "+str(gen_mu_pt_cut_tight)+")"

        cut_mu_status_a = "(GENjpsi_mu_pt[0] && GENpsi2s_mu_pt[0])"
        cut_mu_status_b = "(GENjpsi_mu_pt[0] && GENpsi2s_mu_pt[0])"
    	# GEN level
