...
void loadFile(vector<string>& filenames) {
    // string prefix = "6/DPS_2018_Psi2SJ_";
    // for(int i = 1; i <= 8; i++) {
...
void count() {
    ...
    vector<Double_t> *Jpsi_pt = 0, *Jpsi_y = 0, *psi2S_pt = 0, *psi2S_y = 0, *evt_mass = 0;
    ...
    //     if(!Jpsi_pt->empty() && Jpsi_pt->at(0) > 10 && Jpsi_pt->at(0) < 40 && fabs(Jpsi_y->at(0)) < 2) {
    //         nEvent[0]++;
    ...
    //         }
    //     }
    //     if(!psi2S_pt->empty() && psi2S_pt->at(0) > 10 && psi2S_pt->at(0) < 40 && fabs(psi2S_y->at(0)) < 2) {
    //         nEvent[1]++;
    ...
    // Special task for effective cross section estimation
    Int_t nEntry = ch->GetEntries(), nValid = 0, nFidu = 0, nSingJ = 0, nSingP = 0;
    for(int i = 0; i < nEntry; i++) {
        ch->GetEntry(i);
        if(!(i & 511)) cout<<"Processing: "<<(i+1)<<"th entry\r";
        if(Jpsi_pt->empty() || psi2S_pt->empty()) continue;
        nValid++;
        if(Jpsi_pt->at(0) < 40 && Jpsi_pt->at(0) > 10 && psi2S_pt->at(0) < 40 && psi2S_pt->at(0) > 10 && fabs(Jpsi_y->at(0)) < 2 && fabs(psi2S_y->at(0)) < 2 && evt_mass->at(0) > 7.5) nFidu++;//
        if(Jpsi_pt->at(0) < 40 && Jpsi_pt->at(0) > 10 && fabs(Jpsi_y->at(0)) < 2) nSingJ++;
        if(psi2S_pt->at(0) < 40 && psi2S_pt->at(0) > 20 && fabs(psi2S_y->at(0)) < 1.2) nSingP++;
    }
    cout<<endl<<nEntry<<' '<<nValid<<endl<<nFidu<<' '<<nSingJ<<' '<<nSingP<<endl;
    return;
}
