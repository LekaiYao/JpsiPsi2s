...
def geteffs(channel, List, obs_bins_pt, obs_bins_y, obs_gen_pT, obs_gen_y, genbin_pt, genbin_y):
    ...
    for Sample in List:
        ...
        cut_charm_gen_ya = "(GENjpsi_eta[GEN_pair_id[0].first] > "+str(obs_gen_low_y)+" && GENjpsi_eta[GEN_pair_id[0].first] < "+str(obs_gen_high_y) + ")"
        cut_charm_gen_pta = "(GENjpsi_pt[GEN_pair_id[0].first] > "+str(obs_gen_low_pt)+" && GENjpsi_pt[GEN_pair_id[0].first] < "+str(obs_gen_high_pt) + ")"# a corresponds to J/psi
    	cut_charm_gen_yb = "(GENpsi2s_eta[GEN_pair_id[0].second] > "+str(obs_gen_low_y)+" && GENpsi2s_eta[GEN_pair_id[0].second] < "+str(obs_gen_high_y) + ")"
    	cut_charm_gen_ptb = "(GENpsi2s_pt[GEN_pair_id[0].second] > "+str(obs_gen_low_pt)+" && GENpsi2s_pt[GEN_pair_id[0].second] < "+str(obs_gen_high_pt) + ")"# b corresponds to psi(2S)
        
    	cut_mu_gen_eta_a = "(abs(GENjpsi_mu_eta[GEN_pair_id[0].first][0]) < "+str(gen_mu_eta_cut)+" && abs(GENjpsi_mu_eta[GEN_pair_id[0].first][1]) < "+str(gen_mu_eta_cut)+")"
        cut_mu_gen_pt_a = "(GENjpsi_mu_pt[GEN_pair_id[0].first][0] > "+str(gen_mu_pt_cut)+" && GENjpsi_mu_pt[GEN_pair_id[0].first][1] > "+str(gen_mu_pt_cut)+" && GENjpsi_mu_pt[GEN_pair_id[0].first] > "+str(gen_mu_pt_cut_tight)+")"
    	cut_mu_gen_eta_b = "(abs(GENpsi2s_mu_eta[GEN_pair_id[0].second][0]) < "+str(gen_mu_eta_cut)+" && abs(GENpsi2s_mu_eta[GEN_pair_id[0].second][1]) < "+str(gen_mu_eta_cut)+")"
        cut_mu_gen_pt_b = "(GENpsi2s_mu_pt[GEN_pair_id[0].second][0] > "+str(gen_mu_pt_cut)+" && GENpsi2s_mu_pt[GEN_pair_id[0].second][1] > "+str(gen_mu_pt_cut)+" && GENpsi2s_mu_pt[GEN_pair_id[0].second] > "+str(gen_mu_pt_cut_tight)+")"

        cut_mu_status_a = "(GENjpsi_mu_pt[GEN_pair_id[0].first])"
        cut_mu_status_b = "(GENpsi2s_mu_pt[GEN_pair_id[0].second])"
    	# GEN level
